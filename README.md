...
# Key Features
Fetches and ingests real historical price data (CSV or API-compatible input).
Estimates GBM parameters (drift μ and volatility σ) from log-returns.
Runs thousands of Monte Carlo simulations of future price trajectories.
Produces plots: simulated paths, mean + quantile bands, histogram of terminal prices.
Outputs summary statistics: expected price, median, standard deviation, VaR at chosen confidence levels.
Lightweight, clear code structure suitable for extension (stochastic volatility, jump-diffusions, portfolio simulation).
